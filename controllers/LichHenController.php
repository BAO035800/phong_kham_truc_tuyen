        ...
        try {
            switch ($action) {
                case 'datLich': {
                    ...
                    // 3) Ghép thoi_gian (model cần 'YYYY-MM-DD HH:MM:SS')
                    $gio = $d['gio'];
                    if (strlen($gio) === 5) $gio .= ':00';
                    $d['thoi_gian'] = $d['ngay'] . ' ' . $gio;

                    // 4) Cho phép thiếu ma_dich_vu / ma_phong (model có thể tự xử lý)
                ...
                case 'huyLich': {
                    ...
                    if (!$id) {
                        http_response_code(400);
                        return ['status' => 'error', 'message' => 'Thiếu mã lịch hẹn'];
                    }
                ...
                case 'listByBenhNhan': {
                    ...
                    if (!$ma_benh_nhan) {
                        http_response_code(400);
                        return ['status' => 'error', 'message' => 'Thiếu mã bệnh nhân'];
                    }
                ...
                case 'xacNhanQuaEmail': {
                    ...
                    if (!$token) {
                        http_response_code(400);
                        return ['status' => 'error', 'message' => 'Thiếu token xác nhận'];
                    }
