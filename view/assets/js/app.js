// assets/js/app.js
console.log("[router] ✅ app.js loaded");
...
function runPageHook(hashBase) {
  if (hashBase === "#/register" && typeof window.setupRegisterPage === "function") window.setupRegisterPage();
  if (hashBase === "#/login" && typeof window.setupLoginPage === "function") window.setupLoginPage();
  if (hashBase === "#/confirm" && typeof window.setupConfirmPage === "function") window.setupConfirmPage();
}

/* ----------------- renderer ----------------- */
async function renderPage() {
  console.log("[router] renderPage()", window.location.hash);
  ...
  if (!access.allowed) {
    ...
    if (access.reason === "doctor") {
      root.innerHTML = `<div class="text-center py-10 text-gray-600">⚠️ Chỉ bác sĩ mới được truy cập trang này.</div>`;
      return;
    }
    if (access.reason === "admin") {
      root.innerHTML = `<div class="text-center py-10 text-gray-600">⚠️ Chỉ Admin mới truy cập trang này.</div>`;
      return;
    }
  }
